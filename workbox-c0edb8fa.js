define("https://hijoem.github.io/workbox-c0edb8fa.js", ["exports"], (function(e) {
	"use strict";
	try {
	...
	const a = (e, t) => e.filter(e => t in e),
		u = async ({
			...
			let i = e;
			for (const e of s) i = await e.cacheKeyWillBeUsed.call(e, {
				mode: t,
				request: i
		...
		h = async ({
			...
			for (const t of p) await t.cacheDidUpdate.call(t, {
				cacheName: e,
				event: s,
				oldResponse: y,
				newResponse: w,
		...
		f = async ({
			...
			try {
				...
				for (const e of s) "fetchDidSucceed" in e && (i = await e.fetchDidSucceed.call(e, {
					event: n,
					request: r,
					response: i
				}));
	...
	class y {
		...
		addToCacheList(e) {
			...
			for (const n of e) {
				...
				const {
					cacheKey: e,
					url: s
				} = p(n), i = "string" != typeof n && n.revision ? "reload" : "default";
				if (this.s.has(s) && this.s.get(s) !== e) throw new o("add-to-cache-list-conflicting-entries", {
					firstEntry: this.s.get(s),
					secondEntry: e
				});
				if ("string" != typeof n && n.integrity) {
					if (this.o.has(e) && this.o.get(e) !== n.integrity) throw new o("add-to-cache-list-conflicting-integrities", {
						url: s
					});
		...
		async install({
			...
			for (const [e, t] of this.s) o.has(t) ? s.push(e) : n.push({
				cacheKey: t,
				url: e
			});
			const r = n.map(({
		...
		async u({
			...
			u.redirected && (u = await d(u)), await h({
				event: s,
				plugins: i,
				response: u,
		...
		createHandlerBoundToURL(e, t = !0) {
			if (!this.getCacheKeyForURL(e)) throw new o("non-precached-url", {
				url: e
			});
	...
	function m(e) {
		U || ((({
			...
			self.addEventListener("fetch", s => {
				...
				if (!o) return;
				let r = self.caches.open(c).then(e => e.match(o)).then(e => e || fetch(o));
				s.respondWith(r)
			})
		})(e), U = !0)
	}
	...
	e.precacheAndRoute = function(e, t) {
		! function(e) {
			R().addToCacheList(e), e.length > 0 && (self.addEventListener("install", x), self.addEventListener("activate", K))
		}(e), m(t)
	}
}));
//# sourceMappingURL=workbox-c0edb8fa.js.map